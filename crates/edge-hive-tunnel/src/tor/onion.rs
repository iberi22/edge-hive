...
use futures::{task::SpawnExt, StreamExt, AsyncRead, AsyncWrite};
...
impl OnionService {
    ...
    pub async fn launch(&self) -> Result<String> {
        ...
        let mut config_builder = OnionServiceConfig::builder();
        config_builder.nickname(nickname);
        let config = config_builder.build().unwrap();
        
        let (onion_service, mut receiver) = self.tor_client.launch_onion_service(config).unwrap();
