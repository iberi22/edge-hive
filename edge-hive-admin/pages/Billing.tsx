...
import { UsageMetrics, Subscription } from '../types';
...
const Billing: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const m = await tauriApi.getUsageMetrics();
                const s = await tauriApi.getSubscriptionStatus();
                setMetrics(m);
                setSubscription(s);

                // Simple mock calculation for spend based on usage
                if (m) {
                    // 0.0001 per request, 0.05 per GB storage
                    const calculated = (m.api_requests * 0.0001) + ((m.storage_bytes / 1024 / 1024 / 1024) * 0.05);
                    // Add base plan cost
                    const base = s?.plan === 'Pro' ? 25 : s?.plan === 'Team' ? 100 : 0;
                    setTotalSpent(base + calculated);
                }
            } catch (e) {
                console.error(e);
            } finally {
    ...
    const providerStats = [
        { name: 'Cloudflare', value: totalSpent * 0.2, color: '#f97316', icon: Globe },
        { name: 'AWS', value: totalSpent * 0.4, color: '#eab308', icon: Cloud },
        { name: 'GCP', value: totalSpent * 0.3, color: '#3b82f6', icon: Cpu },
        { name: 'Azure', value: totalSpent * 0.1, color: '#06b6d4', icon: Zap },
    ];
                ...
                <div>
                    ...
                    {subscription && (
                        <span className="text-[10px] font-mono text-slate-400 mt-2 block">
                            Plan: <span className="text-hive-cyan font-bold uppercase">{subscription.plan}</span>
                            <span className="mx-2">|</span>
                            Status: <span className="text-emerald-500 uppercase">{subscription.status}</span>
                        </span>
                    )}
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-slate-900/40 border border-white/5 rounded-xl p-6 backdrop-blur-md relative overflow-hidden">
                            ...
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    <span className="text-2xl font-black text-white">~${(totalSpent * 1.05).toFixed(2)}</span>
                                    <span className="text-[10px] text-slate-500 font-mono ml-2 uppercase">Proj. End of Month</span>
                                </div>
                            </div>
                            <div className="space-y-2">
                                <div className="flex justify-between text-[9px] font-mono text-slate-500 uppercase">
                                    <span>Usage Intensity (Req: {metrics?.api_requests || 0})</span>
                        ...
                        <div className="bg-slate-900/40 border border-white/5 rounded-xl p-6 backdrop-blur-md">
                            ...
                            <div className="p-4 bg-emerald-500/5 border border-emerald-500/20 rounded-lg">
                                ...
                                <p className="text-[11px] text-slate-400 font-mono leading-relaxed italic mb-4">
                                    "Moving {metrics ? (metrics.storage_bytes / 1024 / 1024 / 1024).toFixed(2) : 0} GB from Azure to R2 will reduce annual cost by $1,420.00."
                                </p>
